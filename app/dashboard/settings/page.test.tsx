...
describe('account settings control center', () => {
  ...
  it('renders a wide, navigable settings architecture with live account metrics', async () => {
    ...
    expect(html.match(/Priority queue/g)).toHaveLength(1)
    expect(html).toContain('/api/agent-search?q={query}')
    expect(html).toContain('bg-[var(--fill-1)]')
    expect(html).not.toContain('bg-[var(--panel)]/50')
    expect(html).not.toContain('var(--hover)')
