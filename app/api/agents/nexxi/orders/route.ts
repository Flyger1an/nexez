...
/**
 * GET /api/agents/nexxi/orders - the authenticated buyer's order history.
 *
 ...
 * case-insensitively, and Nexxi purchases stamp that same email at checkout
...
export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, 'agents:nexxi:orders', 30, 60_000)
  ...
  try {
    ...
    console.error('[Nexxi] orders lookup failed', error)
