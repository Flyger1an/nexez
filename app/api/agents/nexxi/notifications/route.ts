...
export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, 'agents:nexxi:notifications', 60, 60_000)
  ...
  const { data, error } = await auth.db
    ...
    .returns<Row[]>()
  if (error) {
    console.error('[Nexxi] notifications list failed', error)
...
export async function PATCH(request: NextRequest) {
  ...
  if (error) {
    console.error('[Nexxi] notifications mark-read failed', error)
