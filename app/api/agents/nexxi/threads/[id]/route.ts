...
/**
 * GET /api/agents/nexxi/threads/[id] - a single conversation's messages, mapped to the
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const limited = await enforceRateLimit(request, 'agents:nexxi:thread', 40, 60_000)
  ...
  try {
    ...
    console.error('[Nexxi] thread load failed', error)
    return NextResponse.json({ error: 'Could not load that conversation.' }, { status: 500 })
  }
}

/**
 * PATCH /api/agents/nexxi/threads/[id] - rename ({ title }) and/or archive ({ archived }).
 * Owner-scoped; the title is trimmed + length-capped server-side.
 */
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const limited = await enforceRateLimit(request, 'agents:nexxi:thread', 30, 60_000)
  if (limited) return limited
  ...
  try {
    ...
    console.error('[Nexxi] thread update failed', error)
