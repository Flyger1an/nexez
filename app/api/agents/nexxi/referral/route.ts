...
export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, 'agents:nexxi:referral', 30, 60_000)
...
export async function POST(request: NextRequest) {
  const limited = await enforceRateLimit(request, 'agents:nexxi:referral', 15, 60_000)
  if (limited) return limited
  ...
  if (error) {
    ...
    console.error('[Nexxi] referral claim failed', error)
