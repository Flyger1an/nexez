...
export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, 'agents:nexie:saved', 40, 60_000)
  ...
  if (error) {
    console.error('[Nexie] saved list failed', error)
...
export async function POST(request: NextRequest) {
  ...
  if (error && (error as { code?: string }).code !== '23505') {
    console.error('[Nexie] save failed', error)
...
export async function DELETE(request: NextRequest) {
  ...
  if (error) {
    console.error('[Nexie] unsave failed', error)
