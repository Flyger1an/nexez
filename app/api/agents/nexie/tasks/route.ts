...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    console.error('[Nexie] tasks list failed', error)
...
export async function POST(request: NextRequest) {
  const limited = await enforceRateLimit(request, 'agents:nexie:tasks', 20, 60_000)
  if (limited) return limited
  ...
  if (error) {
    console.error('[Nexie] create-task failed', error)
...
export async function PATCH(request: NextRequest) {
  ...
  if (error) {
    console.error('[Nexie] update-task failed', error)
...
export async function DELETE(request: NextRequest) {
  ...
  if (error) {
    console.error('[Nexie] delete-task failed', error)
