...
export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, 'agents:nexie:saved-searches', 40, 60_000)
  ...
  if (error) {
    console.error('[Nexie] saved-searches list failed', error)
...
export async function POST(request: NextRequest) {
  ...
  if (error && (error as { code?: string }).code !== '23505') {
    console.error('[Nexie] save-search failed', error)
...
export async function DELETE(request: NextRequest) {
  ...
  if (error) {
    console.error('[Nexie] delete-search failed', error)
