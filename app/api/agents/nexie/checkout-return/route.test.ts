...
vi.mock('../../../../../lib/agents/nexie-auth', () => ({
  authenticateNexieRequest: vi.fn(async () => authRef.ok
...
const request = (sessionId: string) => new Request(
  `https://app.nexez.ai/api/agents/nexie/checkout-return?session_id=${encodeURIComponent(sessionId)}`,
) as never

describe('GET /api/agents/nexie/checkout-return', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    authRef.ok = true
  })
