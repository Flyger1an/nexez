...
import {
  ...
  NexieTurnResponseSchema,
  parseNexieCards,
  parseNexieTurnResponse,
} from './v1'
...
describe('Nexxi v1 wire contract', () => {
  it('matches the published source fingerprint consumed by clients', () => {
    const source = readFileSync(join(process.cwd(), 'contracts', 'nexie', 'v1.ts'))
    expect(createHash('sha256').update(source).digest('hex')).toBe(manifest.sha256)
    expect(manifest).toMatchObject({ contract: 'nexie', version: NEXIE_CONTRACT_VERSION })
  })

  it('validates the current versioned response', () => {
    const response = currentResponse()
    expect(NexieTurnResponseSchema.parse(response)).toEqual(response)
    expect(parseNexieTurnResponse(response).cards[0]).toMatchObject({
  ...
  it('accepts legacy turns only as view-only commerce', () => {
    ...
    expect(parseNexieTurnResponse(legacy)).toMatchObject({
      contractVersion: 1,
