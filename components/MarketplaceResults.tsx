        ...
        {filtered.length > 0 ? filtered.slice(0, 12).map((item: any) => (
          <div key={item.slug} className="card !p-5 hover:border-[var(--signal)]/30 transition">
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-xs">
              ...
              <span className="bg-[var(--amber)]/10 text-[var(--amber)] px-2 py-0.5 rounded">Readiness {item.readiness}</span>
              {item.offer_count > 0 && <span className="text-[#9CA3AF]">{item.offer_count} offers</span>}
              {item.verified && <span className="bg-[var(--ready)]/10 text-[var(--ready)] px-1.5 py-0.5 rounded">✓ Verified</span>}
