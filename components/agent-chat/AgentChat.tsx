  ...
  return (
    ...
    >
      <header className="relative border-b border-white/10 bg-white/[0.035] px-5 pb-4 pt-5 backdrop-blur-2xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            {config.tagline ? <p className="truncate text-sm text-white/55">{config.tagline}</p> : null}
          </div>
        </div>
      </header>
      ...
      <form onSubmit={onSubmit} className="border-t border-white/10 bg-[#09090D]/95 p-4 backdrop-blur-2xl">
        ...
        {config.footnote ? <p className="mt-2 px-2 text-[11px] leading-5 text-white/35">{config.footnote}</p> : null}
      </form>
    </section>
  )
}
