...
describe('CommercialCommandCenter', () => {
  it('renders precise time windows, operational links, and separate currencies', () => {
    ...
    expect(html).toContain('Customer activity · today')
    expect(html).toContain('Orders and deals · now')
    expect(html).toContain('Sales · 30 days')
    expect(html).toContain('/dashboard/commerce')
    expect(html).toContain('4</span>')
    expect(html).toContain('1 urgent · 3 deals')
    expect(html).toContain('USD')
    expect(html).toContain('JPY')
    expect(html).toContain('A single order can appear in more than one task category')
  ...
  it('shows unavailable sources as unavailable rather than false zeroes', () => {
    ...
    expect(html).toContain('Your commerce tasks are temporarily unavailable')
    expect(html).toContain('Finance totals are temporarily unavailable.')
    expect(html).toContain('Some totals are unavailable')
    expect(html).toContain('cannot confirm that you are all caught up')
  })
})
