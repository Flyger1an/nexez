...
// the card types + renderers, the /api/agents/nexxi endpoint wiring, and the
...
export function NexxiChat({ initialThreadId, className = '' }: NexxiChatProps) {
  const threadIdRef = useRef<string | undefined>(initialThreadId)

  async function postTurn(body: Record<string, unknown>): Promise<AgentTurnResponse<NexxiCard>> {
    const response = await fetch('/api/agents/nexxi', {
      method: 'POST',
  ...
  async function streamTurn(
  ...
  ): Promise<AgentTurnResponse<NexxiCard>> {
    let response: Response
    try {
      response = await fetch('/api/agents/nexxi/stream', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
    ...
    let buffer = ''
    let final: AgentTurnResponse<NexxiCard> | null = null
    ...
    const handleFrame = (frame: string) => {
      ...
      if (!payload) return
      let evt: { type?: string; value?: string; error?: string; threadId?: string; message?: string; cards?: NexxiCard[] }
      try {
  ...
  function decide(
    controller: AgentChatController<NexxiCard>,
    card: Extract<NexxiCard, { type: 'approval' }>,
  ...
  return (
    <AgentChat<NexxiCard>
      ...
      config={{
        ...
        quickPromptEvent: 'nexxi:quick-prompt',
        ...
        renderCard: (card, controller) => (
          <NexxiCardView card={card} onDecision={(c, decision) => decide(controller, c, decision)} />
        ),
      }}
    />
  )
...
function NexxiCardView({
  card,
  onDecision,
}: {
  card: NexxiCard
  onDecision: (card: Extract<NexxiCard, { type: 'approval' }>, decision: 'approved' | 'rejected') => void
}) {
              ...
              {card.checkoutUrl ? (
                <button
                  ...
                  onClick={() => {
                    ...
                    window.dispatchEvent(new CustomEvent('nexxi:quick-prompt', { detail: prompt }))
                  }}
                >
                  Ask Nexxi to book
                </button>
