 ...
 *
 * Allows pasting ANY competitor website URL (not limited to Nexez listings).
 ...
 *  - Optional side-by-side vs user's own Nexez listing
 *
...
export type CompetitorAnalysis = {
  ...
  // Optional side-by-side (when user provides their Nexez listing slug)
  userComparison?: {
    slug: string
    readiness: number
    trust: number
...
export async function analyzeCompetitorSite(
  ...
  // Attach side-by-side comparison if caller supplied a Nexez listing summary
  if (opts?.userNexezPage) {
...
export function analysisToMarkdown(a: CompetitorAnalysis): string {
  ...
  if (a.userComparison) {
    md += `## Side-by-Side vs Your Nexez Listing (/${a.userComparison.slug})\n`
  ...
  md += `---\nGenerated by Nexez Competitor Website Analyzer. Use these insights to improve your agent-optimized listing.`
  return md
}
