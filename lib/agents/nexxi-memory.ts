...
export type NexxiMemoryTiming = 'flexible' | 'this_week' | 'asap'

export type NexxiMemorySignals = {
  /** Major currency units; only set when an explicit ceiling cue precedes a $ amount. */
  budgetObserved: number | null
  timingObserved: NexxiMemoryTiming | null
...
const TIMING_RULES: [RegExp, NexxiMemoryTiming][] = [
...
export function extractMemorySignals(message: string): NexxiMemorySignals {
  ...
  let timingObserved: NexxiMemoryTiming | null = null
