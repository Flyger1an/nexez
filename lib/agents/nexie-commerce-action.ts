...
import { z } from 'zod'
import {
  NexieCommerceRailSchema,
  type NexieCommerceRail,
...
const INTERNAL_PREPARED_ACTION_KEY = '__nexiePreparedAction'
...
export type NexieCommerceActionDescriptor = {
  schemaVersion: 1
...
type ResolvedAction = {
  rail: Exclude<NexieCommerceRail, 'provider' | 'negotiation' | 'external' | 'unknown'>
...
export async function resolveNexieCommerceAction(
  db: SupabaseClient,
  ...
  if (!slug || !offerKey) {
    throw new NexieCommercePreparationError('A valid listing and offer are required.', 'invalid_contract')
  }
  ...
  if (error || !page || !isPublicLaunchVisiblePage(page)) {
    throw new NexieCommercePreparationError('This offer is not available in Nexxi.', 'not_available')
  }

  const offer = getCheckoutOffer(page, offerKey)
  if (!offer || !isOfferActionAvailable(offer)) {
    throw new NexieCommercePreparationError('This offer is not available for checkout right now.', 'not_available')
  }
  if (getPreferredOriginalOfferUrl(page, offer)) {
    throw new NexieCommercePreparationError('This offer must be completed on the provider site.', 'provider_handoff')
  }
  if (offer.offerType === 'negotiable') {
    throw new NexieCommercePreparationError('Use the negotiation action for this offer.', 'negotiation_required')
  }
  if ((parseMarketplacePriceCents(offer.price) ?? 0) <= 0) {
    throw new NexieCommercePreparationError('This offer does not have a payable price.', 'not_available')
  }
...
export async function prepareNexieBookingAction(
  db: SupabaseClient,
  payload: Record<string, unknown>,
  buyer: Buyer,
  ...
  descriptor: NexieCommerceActionDescriptor
}> {
  const resolved = await resolveNexieCommerceAction(db, payload)
  const missingInputs = missingRequiredBuyerInputs(resolved.inputSchema, payload.offerConfiguration)
  if (missingInputs.length) {
    throw new NexieCommercePreparationError(
  ...
  const descriptor: NexieCommerceActionDescriptor = {
    schemaVersion: 1,
    rail: resolved.rail,
  ...
  return {
    storedPayload: {
      ...
      ...(payload.__nexieCommerce ? { __nexieCommerce: payload.__nexieCommerce } : {}),
...
export async function validatePreparedNexieBookingAction(
  db: SupabaseClient,
  payload: Record<string, unknown>,
): Promise<PreparedBooking> {
  const prepared = parsePrepared(payload)
  const current = await resolveNexieCommerceAction(db, prepared.input)
...
export async function executePreparedNexieBookingAction(
  payload: Record<string, unknown>,
  options: { baseUrl: string; fetchImpl?: typeof fetch },
) {
...
export function hasPreparedNexieBookingAction(payload: Record<string, unknown>): boolean {
