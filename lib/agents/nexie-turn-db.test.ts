...
import { createNexieTurnDb } from './nexie-turn-db'
...
describe('createNexieTurnDb', () => {
  it('keeps normal buyer data on the user-scoped client', () => {
    ...
    const db = createNexieTurnDb(user.client, approvalFactory)
  ...
  it('preserves method binding for the underlying user client', () => {
    const user = fakeClient('user')
    const db = createNexieTurnDb(user.client, () => fakeClient('admin').client)
