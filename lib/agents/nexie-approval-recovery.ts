...
// Finish Nexie actions that were claimed but never completed.
//
...
//
// RETRY RATHER THAN FAIL. The action is replay-safe: `nexie:<id>:approved-action` is
  ...
  for (const row of rows) {
    ...
    if ((row.recovery_attempts ?? 0) + 1 > MAX_RECOVERY_ATTEMPTS) {
      await admin
        ...
        .eq('id', row.id)
      summary.failed += 1
      captureEvent('nexie.action.recovery', { tool: row.tool_name, outcome: 'exhausted' })
      continue
    }
    ...
    try {
      ...
      const idempotencyKey = `nexie:${row.id}:approved-action`
      const result =
      ...
      await admin
        ...
        .eq('id', row.id)
      summary.recovered += 1
      captureEvent('nexie.action.recovery', { tool: row.tool_name, outcome: 'recovered' })
    } catch (err) {
      ...
      captureError(err, { scope: 'nexie.action.recovery', tool: row.tool_name, approvalId: row.id })
      captureEvent('nexie.action.recovery', { tool: row.tool_name, outcome: 'retry_pending' })
      summary.skipped += 1
    }
  }
