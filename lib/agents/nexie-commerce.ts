...
import {
  NexieCommerceCapabilitySchema,
  type NexieCommerceCapability,
...
const APPROVAL_COMMERCE_KEY = '__nexieCommerce'

function capability(
  state: NexieCommerceCapability['state'],
  rail: NexieCommerceRail,
  reasonCode: NexieCommerceCapability['reasonCode'],
  message: string,
): NexieCommerceCapability {
  return { state, rail, reasonCode, message }
}

function preparedRail(rail: Extract<NexieCommerceRail, 'configured' | 'recurring' | 'staged' | 'reservable'>): NexieCommerceCapability {
...
export function commerceCapabilityForSearchResult(result: AgentSearchResult): NexieCommerceCapability {
...
function railForPageOffer(page: AgentPage, offerKey: string): {
  capability: NexieCommerceCapability
  offerFound: boolean
} {
...
export async function resolveNexieBookingCapability(
  db: SupabaseClient,
  payload: Record<string, unknown>,
): Promise<NexieCommerceCapability> {
...
export function attachApprovalCommerce(
  payload: Record<string, unknown>,
  commerce: NexieCommerceCapability,
): Record<string, unknown> {
  return { ...payload, [APPROVAL_COMMERCE_KEY]: commerce }
}

export function approvalCommerce(payload: Record<string, unknown>): NexieCommerceCapability | null {
  const parsed = NexieCommerceCapabilitySchema.safeParse(payload[APPROVAL_COMMERCE_KEY])
