...
export type NexxiTiming = 'flexible' | 'this_week' | 'asap'

/** Buyer-facing push categories the buyer can independently mute (master switch = notificationsEnabled). */
export type NexxiNotificationCategory = 'orders' | 'alerts' | 'tasks'
export type NexxiNotificationTypes = Record<NexxiNotificationCategory, boolean>
export const NEXXI_NOTIFICATION_CATEGORIES: NexxiNotificationCategory[] = ['orders', 'alerts', 'tasks']

export type NexxiPreferences = {
  ...
  notificationTypes: NexxiNotificationTypes
  /**
...
export const NEXXI_TIMINGS: NexxiTiming[] = ['flexible', 'this_week', 'asap']
...
export const DEFAULT_PREFERENCES: NexxiPreferences = {
  budgetMax: null,
  currency: 'USD',
  categories: [],
  timing: null,
...
/** Coerce arbitrary stored/posted JSON into a complete, safe NexxiPreferences. */
export function normalizePreferences(input: unknown): NexxiPreferences {
  ...
  const timing = NEXXI_TIMINGS.includes(o.timing as NexxiTiming) ? (o.timing as NexxiTiming) : null
  ...
  const notificationTypes: NexxiNotificationTypes = {
...
const TIMING_LABEL: Record<NexxiTiming, string> = {
 ...
 */
export function preferencesPromptBlock(prefs: NexxiPreferences): string {
  const lines: string[] = []
