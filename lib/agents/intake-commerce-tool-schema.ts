...
export const OFFER_INPUT_TOOL_SCHEMA = {
  ...
  properties: {
    ...
    pricing: {
      type: 'object',
      description:
        'Optional deterministic merchant-confirmed price delta. Never infer amounts. The listing currency is implied. Supported only for select, boolean, and quantity inputs; the reducer validates the model against valueType/options.',
      properties: {
        model: {
          type: 'string',
...
- Price amounts are signed major-unit strings in the listing currency: "25" means +25, "-10" means a 10-unit discount. If pricing is present, affects MUST include "price".
