...
/**
 * Grok (xAI) Adapter - uses OpenAI-compatible API + tools/function calling.
 */
export class GrokAdapter extends BaseLLMAdapter {
  private client: OpenAI;
  readonly provider = 'grok';

  constructor(apiKey: string, model = 'grok-2', baseUrl = 'https://api.x.ai/v1') {
    super();
    this.model = model;
  ...
  async negotiate(rules: any, proposal: any, history: any[]): Promise<NegotiationDecision> {
    ...
    try {
      ...
      if (!toolCall?.function) throw new Error('No function call returned');

      const args = JSON.parse(toolCall.function.arguments || '{}');
      return this.parseFunctionCall(toolCall.function.name, args);
    } catch (err: any) {
      throw new LLMAdapterError(`Grok negotiation failed: ${err.message}`, this.provider);
    }
  }
...
accept_proposal
 ...
 - scope_excluded: string (optional: revised excluded scope)
 - max_revisions: number (optional)
 - max_project_weeks: number (optional)
 - scheduling_link: string (optional: the concrete Calendly/booking link the agent should use to lock the date)
    ...
    return [
      {
        ...
        function: {
          ...
          description: 'Accept when proposal complies with rules.',
          parameters: {
            type: 'object',
            properties: {
      ...
      },
      {
        ...
        function: {
          ...
          parameters: {
            type: 'object',
            properties: { reasoning: { type: 'string' }, internal_notes: { type: 'string' } },
            required: ['reasoning'],
          },
        },
      },
  ...
  private parseFunctionCall(name: string, args: any): NegotiationDecision {
    ...
    if (name === 'generate_counter_offer') {
      decision.counter = {
        ...
        // also hoist scheduling if provided at counter time
      };
      if (args.scheduling_link) {
        decision.schedulingLink = args.scheduling_link;
      }
    }
    if (name === 'request_clarification' && args.questions) {
      decision.clarificationQuestions = args.questions;
    }
    if (args.scheduling_link && !decision.schedulingLink) {
      decision.schedulingLink = args.scheduling_link;
    }
    // Top-level scope for non-counter decisions if LLM supplies (via other functions or future)
