 ...
 */
export function isPublicLaunchVisiblePage(page: Pick<AgentPage, 'slug'>): boolean {
  const slug = page.slug?.trim() || ''
  if (!slug) return false
  return !INTERNAL_SEED_SLUGS.some((pattern) => pattern.test(slug))
}

export function publicLaunchVisiblePages<T extends Pick<AgentPage, 'slug'>>(pages: T[] | null | undefined): T[] {
