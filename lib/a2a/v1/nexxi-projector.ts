...
export type NexxiProjectableEvent =
  | {
      type: 'text-delta'
      delta: string
      source: 'model' | 'replay'
  ...
  | {
      type: 'completed'
      result: NexxiTurnPayload
    }

export type NexxiA2AV1ProjectionIds = {
  taskId: string
  contextId: string
  artifactId?: string
}

type PublicNexxiCard = Record<string, unknown>
type Clock = () => string
...
function terminalState(result: NexxiTurnPayload): A2AV1TaskState {
  if (
    result.cards.some(
 ...
 */
function publicCard(card: NexxiCard): PublicNexxiCard {
 ...
 */
export class NexxiA2AV1Projector {
  ...
  project(event: NexxiProjectableEvent): A2AV1StreamResponse[] {
  ...
  snapshot(result: NexxiTurnPayload): A2AV1Task {
    return {
      id: this.taskId,
      contextId: this.contextId,
      status: {
