...
import { NexxiA2AStreamProjector } from './nexxi-stream'

const baseResult: NexxiTurnResult = {
  threadId: 'thread-1',
  agentId: 'agent-1',
  message: 'Found one.',
  cards: [],
...
describe('NexxiA2AStreamProjector', () => {
  it('uses replace-then-append semantics for progressive preview text', () => {
    const projector = new NexxiA2AStreamProjector({
      taskId: 'task-1',
      contextId: 'context-1',
  ...
  it('ends an approval turn in input-required rather than claiming completion', () => {
    ...
    const approvalResult: NexxiTurnResult = {
      ...baseResult,
      cards: [
        {
          type: 'approval',
  ...
  it('marks an unsuccessful Nexxi action as a failed A2A task', () => {
    ...
    const failedResult: NexxiTurnResult = {
      ...baseResult,
