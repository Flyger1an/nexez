import { NEXXI_CONTRACT_VERSION } from '../../contracts/nexxi/v1'
import type { NexxiExecutionEvent } from '../agents/nexxi-stream'
...
export type NexxiA2AStreamEvent =
  | A2ATaskArtifactUpdateEvent
  | A2ATaskStatusUpdateEvent

export type NexxiA2AStreamIds = {
  taskId: string
  contextId: string
  artifactId?: string
}

function terminalState(result: NexxiTurnResult): A2ATaskState {
  if (
    result.cards.some(
 ...
 * authoritative.
 */
export class NexxiA2AStreamProjector {
  private previewStarted = false
  private readonly artifactId: string

  constructor(private readonly ids: NexxiA2AStreamIds) {
    this.artifactId = ids.artifactId ?? `${ids.taskId}:nexxi-response`
  }

  project(event: NexxiExecutionEvent): NexxiA2AStreamEvent[] {
