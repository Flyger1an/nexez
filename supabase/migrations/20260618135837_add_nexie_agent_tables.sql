-- Nexie: buyer-facing personal agent memory, chat threads, and approval ledger.
...
create table if not exists public.user_agents (
  ...
  name text not null default 'Nexie',
...
create table if not exists public.agent_threads (
  ...
  user_id uuid not null references auth.users(id) on delete cascade,
  title text not null default 'New Nexie chat',
...
drop policy if exists "Users manage own Nexie agent" on public.user_agents;
create policy "Users manage own Nexie agent"
...
drop policy if exists "Users manage own Nexie threads" on public.agent_threads;
create policy "Users manage own Nexie threads"
...
drop policy if exists "Users manage own Nexie messages" on public.agent_messages;
create policy "Users manage own Nexie messages"
...
create policy "Users manage own Nexie approvals"
...
create or replace function public.nz_touch_nexie_updated_at()
returns trigger
language plpgsql
...
revoke execute on function public.nz_touch_nexie_updated_at() from public, anon, authenticated;
...
create trigger trg_touch_user_agents_updated_at
  ...
  for each row
  execute function public.nz_touch_nexie_updated_at();
...
comment on table public.user_agents is 'Buyer-owned Nexie profile, preferences, and durable memory.';
comment on table public.agent_threads is 'Mobile-first Nexie chat threads for buyer discovery and transactions.';
comment on table public.agent_messages is 'Conversation transcript for Nexie text and voice turns.';
comment on table public.agent_action_approvals is 'Explicit user approval ledger before Nexie negotiates, books, or starts checkout.';
