-- Close the Nexxi approval-boundary bypass.
--
...
drop policy if exists "Users read own Nexxi approvals" on public.agent_action_approvals;

create policy "Users read own Nexxi approvals"
...
begin
  ...
  if thread_owner is null or thread_owner is distinct from new.user_id then
    raise exception using
      errcode = '23514',
      message = 'Nexxi approval user must match the owning thread.';
  end if;
  ...
  if new.tool_name not in ('initiate_negotiation', 'trigger_booking')
    ...
    raise exception using
      errcode = '23514',
      message = 'Nexxi approval action is outside the supported contract.';
  end if;
  ...
  if tg_op = 'INSERT' then
    if new.status <> 'PENDING'
      ...
      raise exception using
        errcode = '23514',
        message = 'Nexxi approval inserts must start as pristine PENDING rows.';
    end if;
  ...
  if new.id is distinct from old.id
    ...
    raise exception using
      errcode = '23514',
      message = 'Nexxi approval identity and proposed action are immutable.';
  end if;

  if old.status in ('REJECTED', 'EXECUTED', 'FAILED') then
    raise exception using
      errcode = '23514',
      message = 'Terminal Nexxi approvals are immutable.';
  end if;
  ...
  if old.status = 'PENDING' then
    if new.status not in ('APPROVED', 'REJECTED')
      ...
      raise exception using
        errcode = '23514',
        message = 'Pending Nexxi approvals may only be approved or rejected.';
    end if;
  ...
  if old.status = 'APPROVED' then
    if new.decided_at is distinct from old.decided_at then
      raise exception using
        errcode = '23514',
        message = 'Nexxi approval decision time is immutable.';
    end if;
    ...
    if new.status = 'APPROVED' then
      if new.result is distinct from old.result
        ...
        raise exception using
          errcode = '23514',
          message = 'Approved Nexxi actions may only advance the recovery lease.';
      end if;
    ...
    if new.status = 'EXECUTED' then
      if new.completed_at is null
        ...
        raise exception using
          errcode = '23514',
          message = 'Executed Nexxi approvals require one complete result.';
      end if;
    ...
    if new.status = 'FAILED' then
      if new.completed_at is null
        ...
        raise exception using
          errcode = '23514',
          message = 'Failed Nexxi approvals require one complete error outcome.';
      end if;
    ...
    raise exception using
      errcode = '23514',
      message = 'Approved Nexxi actions may only be executed, failed, or recovery-leased.';
  end if;

  raise exception using
    errcode = '23514',
    message = 'Nexxi approval has an invalid current state.';
end;
$$;
