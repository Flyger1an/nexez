...
alter table public.checkout_configuration_handoffs
  drop constraint if exists checkout_configuration_handoffs_pricing_snapshot_check,
  add constraint checkout_configuration_handoffs_pricing_snapshot_check
    check (
      pricing_snapshot is null
      or (
        jsonb_typeof(pricing_snapshot) = 'object'
        and octet_length(pricing_snapshot::text) <= 65536
      )
    ),
  drop constraint if exists checkout_configuration_handoffs_pricing_fingerprint_check,
  add constraint checkout_configuration_handoffs_pricing_fingerprint_check
    check (
      pricing_fingerprint is null
      or pricing_fingerprint ~ '^[a-f0-9]{64}$'
    ),
  drop constraint if exists checkout_configuration_handoffs_pricing_pair_check,
  add constraint checkout_configuration_handoffs_pricing_pair_check
    check ((pricing_snapshot is null) = (pricing_fingerprint is null));

comment on column public.checkout_configuration_handoffs.pricing_snapshot is
  'Exact deterministic checkout-time pricing provenance in Stripe smallest units: base amount, merchant-authored adjustments, and final amount.';
comment on column public.checkout_configuration_handoffs.pricing_fingerprint is
  'SHA-256 fingerprint of the deterministic checkout-time pricing snapshot.';
