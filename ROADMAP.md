...
- **Test coverage** — beyond the `lib/` unit suite (297 tests total): **API route-handler tests** (vitest, mocked Supabase via `test/supabase-mock.ts`) covering auth/tenancy/gating across api/v1 (+`[id]`), negotiations, account delete/export, custom-domain verify, Stripe webhook, billing checkout, public-simulate, and the `agent.json`/`llms.txt` artifacts; **component tests** (jsdom + Testing Library: ThemeToggle, ErrorBoundary, VisualOfferBuilder, PageCard); and **Playwright E2E** (`npm run test:e2e`) — public-page smoke + an authed, non-destructive editor smoke (creds via env, skips in CI). Responsive layout verified at mobile across all routes (no horizontal overflow).
