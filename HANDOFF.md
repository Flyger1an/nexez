...
_Last updated: 2026-06-05 (long-tail tests + CI) · HEAD `70012fe` (verify with `git rev-parse --short main`)_
...
- `origin/main` == local @ **`70012fe`**, clean tree (except untracked `.github/workflows/` — see below). Vercel auto-deploys on push; prod verified live (`/icon.png` + `/nexez-logo.png` → 200; tab title "Nexez — Pages built for AI agents", favicon `/icon.png`). The editor route `/dashboard/[id]` is now a **server component** (was a `'use client'` monolith).
...
- ⚠️ **CI workflows authored but NOT pushed** — `.github/workflows/ci.yml` (lint/tsc/test/build on push+PR) and `e2e.yml` (manual Playwright) are on disk, **untracked**. The push token lacks the GitHub **`workflow` scope**, so workflow files are rejected. To land them: push with a `workflow`-scoped token, or paste the two files via the GitHub UI.
...
- **Long-tail tests + CI** (`70012fe`) — +14 tests (311 total): routes `dashboard/api-keys` (owner-bound key, hash-only storage, raw once), `webhooks/calendly` (service-role/slug/page gating), `[slug]/badge.json`, `[slug]/mcp.json` (mcp_enabled gating); components `ReadinessAside`, `EditorToolbar`. **Authored `.github/workflows/ci.yml` + `e2e.yml`** (CI gate on push/PR + manual Playwright) — held back: the push token has no `workflow` scope (files sit untracked on disk).
